...
from alpha2.expression.tokens import (
    RegisterToken,
    NullToken,
    null_token_idx,
    operand_tokens,
...
def get_all_ternary_actions(
    start_idx, end_idx, operator_tks, operand_tks, action_shift
):
    action_list, token_action_list = [], []
    for i in range(start_idx, end_idx):
        operator_tk = operator_tks[i]
...
def initialize_ternary_actions(operator_tks, operand_tks, action_shift, num_splits=1):
    ...
    for i in range(num_splits):
        ...
        if i == num_splits - 1:
            end_index = num_operator_tokens
        refs.append(
            get_all_ternary_actions.remote(
                start_index, end_index, operator_tks, operand_tks, action_shift
            )
        )
    action_lists = ray.get(refs)
...
refs = [
    initialize_UNARY_actions.remote(UNARY_OP_TOKENS, operand_tokens, null_token_idx),
    initialize_BINARY_actions.remote(
        BINARY_OP_TOKENS, operand_tokens, action_shift=len(UNARY_OP_TOKENS)
    ),
    initialize_ternary_actions.remote(
        TERNARY_OP_TOKENS,
        operand_tokens,
        action_shift=len(UNARY_OP_TOKENS) + len(BINARY_OP_TOKENS),
    ),
]
(
    [unary_actions, unary_token_actions],
    [binary_actions, binary_token_actions],
    [ternary_actions, ternary_token_actions],
) = ray.get(refs)


start_action = tuple([start_token_idx, null_token_idx, null_token_idx, null_token_idx])
finish_action = tuple(
    [finish_token_idx, null_token_idx, null_token_idx, null_token_idx]
)

all_action_list = (
    [start_action, finish_action] + unary_actions + binary_actions + ternary_actions
)
start_action_idx = 0
finish_action_idx = 1
...
register_token_ids = [
    i for i in range(NUM_OPERANDS) if isinstance(operand_tokens[i], RegisterToken)
]
